...
@section('content')
  <style>
    ...
    .thumb { width:110px; height:110px; object-fit:cover; border-radius:.35rem; border:1px solid #e5e7eb; }
  ...
  <h1 style="margin-bottom:.75rem;">Item bewerken</h1>
  ...
  <form method="POST"
    ...
    <div>
      ...
      <div class="form-grid">
        <div class="field">
          ...
          <input id="title" type="text" name="title" value="{{ old('title', $item->title) }}" class="control" required>
        </div>

        <div class="field">
          <label for="condition">Conditie</label>
          <input id="condition" type="text" name="condition" value="{{ old('condition', $item->condition) }}" class="control" placeholder="bijv. gebruikt">
        </div>

        <div class="field" style="grid-column:1 / -1;">
          <label for="description">Omschrijving</label>
          <textarea id="description" name="description" class="control" rows="4">{{ old('description', $item->description) }}</textarea>
        </div>

        <div class="field">
          <label for="location">Locatie</label>
          <input id="location" type="text" name="location" value="{{ old('location', $item->location) }}" class="control">
        </div>

        <div class="field">
          <label for="price_per_day">Prijs per dag (€)</label>
          <input id="price_per_day" type="text" name="price_per_day" value="{{ old('price_per_day', $item->price_per_day) }}" class="control">
        </div>

        <div class="field">
          <label for="status">Status</label>
          @php $st = old('status', $item->status); @endphp
        ...
        <div class="field">
          ...
          {{-- Admin-only inline category add --}}
    ...
    <div>
      ...
      <div class="form-grid">
        <div class="field">
          ...
          <div class="muted-xs">Laat leeg om de huidige afbeelding te behouden.</div>

          @if ($item->cover_image)
            <div style="margin-top:.5rem;">
              <div class="muted-xs">Huidige afbeelding:</div>
              <img src="{{ asset('storage/'.$item->cover_image) }}"
                   alt="Huidige omslag"
                   style="max-width:240px; height:auto; border-radius:.4rem;">
            </div>
          @endif
        </div>
        ...
        <div class="field">
          ...
          <div class="muted-xs">Nieuwe bestanden worden toegevoegd aan de galerij.</div>

          @php $item->loadMissing('images'); @endphp
          @if ($item->images->count())
            <div style="margin-top:.5rem; display:flex; gap:.5rem; flex-wrap:wrap;">
              @foreach ($item->images as $img)
                <img src="{{ asset('storage/'.$img->path) }}" alt="Afbeelding" class="thumb">
              @endforeach
            </div>
          @endif
        </div>
    ...
    <div class="actions" style="margin-top:.25rem;">
      <button class="btn-add" type="submit">Opslaan</button>
